...
class SensorService:
    ...
    def start(self):
        ...
        if self.config.cell_locator_token:
    ...
    def _sensor_worker(self):
        while True:
            ...
            if self.adc:
                try:
                    ...
                    if power is not None:
                        self.power = power
                    values["BatteryVoltage"] = self.battery
                    values["PowerVoltage"] = self.power
                except Exception as error:
                    self.logger.warn(
            ...
            if values:
                try:
                    self.cloud.publish_properties(values)
                except Exception as error:
                    self.logger.warn(
    ...
    def _location_worker(self):
        ...
        # Preserve the existing 24-hour reporting interval without hardcoding a token.
        while True:
            utime.sleep(86400)
            try:
                location = cellLocator.getLocation(
                    "www.queclocator.com", 80, self.config.cell_locator_token, 8, 1
                )
                self.cloud.publish_properties(
                    {
                        "CellLocator": {
