...
productKey = "k1qaiXzWevM"  # 产品标识(参照阿里 IoT 平台应用开发指导)
productSecret = None  # 产品密钥（使用一机一密认证时此参数传入None，参照阿里 IoT 平台应用开发指导)
DeviceName = "lexio"  # 设备名称(参照阿里 IoT 平台应用开发指导)
DeviceSecret = "2c028691b5a32c898b41feeab7b05fda"  # 设备密钥（使用一型一密认证此参数传入None，免预注册暂不支持，需先在 IoT 平台创建设备，参照阿里 IoT 平台应用开发指导)
...
if __name__ == '__main__':
    ...
    if stagecode == 3 and subcode == 1:
        ...
        # 设置mqtt连接属性
        clientID = "lsl"  # 自定义字符（不超过64）
        ali.setMqtt(clientID, clean_session=False, keepAlive=300)

        # 设置回调函数
        ali.setCallback(sub_cb)
        topic = "/k1qaiXzWevM/lexio/user/get"  # IoT 平台自定义或自拥有的Topic
        # 订阅主题
        ali.subscribe(topic)
        # 发布消息
