...
def preallocate(path, size):
    ...
    try:
        ...
        # A failed oversized reservation must not consume the small EC600M
        # filesystem indefinitely. Existing valid queues never use this
        # helper to resize in place.
...
class DeliveryStore:
    ...
    def oldest_sequence(self):
        ...
        if ram_record and flash_record:
            return min(ram_record["seq"], flash_record["seq"])
    ...
    def total_depth(self):
        return self.ram.depth() + self.flash_depth()

    def occupancy_percent(self):
        capacity = self.total_capacity()
        if capacity <= 0:
            return 100
        return (self.total_depth() * 100) // capacity

    def available_frames(self):
        return max(0, self.total_capacity() - self.total_depth())

    def flash_available_frames(self):
        if not self.journal:
            return 0
    ...
    def detach_empty_journal(self):
        ...
        try:
            ...
            try:
                if journal.count:
                    return False
                paths = (journal.data_path,) + journal.meta_paths
                journal.capacity = 0
                self.journal = None
            finally:
                journal.lock.release()
        ...
        result = True
        for path in paths:
            if file_size(path) >= 0 and not remove_file(path):
                result = False
        return result
