...
class QueueTests(unittest.TestCase):
    ...
    def test_ota_discard_clears_ram_flash_and_silently_sinks_new_frames(self):
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            journal = FlashJournal(
                str(root / "queue.bin"),
            ...
            self.assertTrue(delivery.accept(frame(1), 1))
            self.assertEqual(delivery.spill(force=True), 1)
            self.assertTrue(delivery.accept(frame(2), 2))
            self.assertEqual(delivery.discard_all(), 2)
            self.assertEqual(delivery.total_depth(), 0)
            self.assertFalse(delivery.accepting)
            self.assertTrue(delivery.accept(frame(3), 3))
            self.assertEqual(delivery.total_depth(), 0)
            self.assertEqual(delivery.stats()["discarded"], 3)
            delivery.resume_accepting()
            self.assertTrue(delivery.accept(frame(4), 4))
